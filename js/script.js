...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Xử lý translation cho trang transportation
  const initTransportationPage = () => {
    // Xử lý translation cho form inputs
    const translatePlaceholders = () => {
      document.querySelectorAll('[data-translate-placeholder]').forEach(input => {
        const key = input.getAttribute('data-translate-placeholder');
        const translation = languageSwitcher.getNestedTranslation(
          languageSwitcher.getCurrentTranslations(),
          key
        );
        if (translation) {
    ...
    // Xử lý translation cho select options
    ...
    // Xử lý translation cho service details
    const translateServiceDetails = () => {
      ...
      // Cập nhật service details dựa trên ngôn ngữ
      document.querySelectorAll('.service-card').forEach(card => {
        const title = card.querySelector('h3').textContent;
        const description = serviceDetails[title]?.description;
        if (description) {
          card.querySelector('.service-description').innerHTML = description;
        }
      });
    };

    // Xử lý khi chuyển đổi ngôn ngữ
    ...
    // Đăng ký sự kiện language change
    document.addEventListener('languageChanged', handleLanguageChange);

    // Khởi tạo translation ban đầu
    handleLanguageChange();
  };

  // Kiểm tra nếu đang ở trang transportation
  if (document.querySelector('.transport-container')) {
